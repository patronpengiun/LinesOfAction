...
import linesOfAction.client.GameApi.VerifyMoveDone;
import linesOfAction.client.GameApi.EndGame;
import linesOfAction.client.GameApi.Operation;
import linesOfAction.client.GameApi.Set;
import linesOfAction.client.GameApi.SetTurn;
...
public class CheatLogic {
	...
	void check(Map<String, Object> lastState, List<Operation> lastMove, int lastMovePlayerId, List<Map<String, Object>> playersInfo){
		for (;;){
			...
			if ((lastMove.size()==4) && (!checkEndGame(lastState,lastMove,lastMovePlayerId,playersInfo))) {System.out.println(10);break;}
	...
	boolean checkDestinationColor(Map<String, Object> lastState, List<Operation> lastMove){
		Set moveTo = (Set)lastMove.get(1);
		if (moveTo.getValue() == lastState.get("turn")) return true;
		else return false;
	}
	
	boolean checkSetTurn(Map<String, Object> lastState, List<Operation> lastMove, List<Map<String, Object>> playersInfo){
		SetTurn setTurn = (SetTurn)lastMove.get(2); 
		Object wId = playersInfo.get(0).get("playerId");
		int w_id = ((Integer) wId).intValue();
		String turn = setTurn.getPlayerId()==w_id ? "W" : "B";
		if (turn != lastState.get("turn")) return true;
		else return false;
	}
	...
	boolean checkEndGame(Map<String, Object> lastState, List<Operation> lastMove,int lastMovePlayerId, List<Map<String, Object>> playersInfo){
		EndGame endGame = (EndGame)lastMove.get(3);
		Object WID = playersInfo.get(0).get("playerId");
		Object BID = playersInfo.get(1).get("playerId");
		int w_id = ((Integer)WID).intValue();
		int b_id = ((Integer)BID).intValue();
		String winner;
		...
		if (!(white_win || black_win)) return false;
			...
			if (winner == "B" && lastMovePlayerId == b_id) return true;
			if (winner == "W" && lastMovePlayerId == w_id) return true;
			return false;
		}
		else return false;
	}
