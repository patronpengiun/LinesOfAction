...
import linesOfAction.client.GameApi.Container;
...
public class GamePresenter {
	...
	public void updateUI(UpdateUI updateUI) {
		...
		int lastMovePlayerId = updateUI.getLastMovePlayerId();
		...
		if (-1 != yourPlayerIndex){
			...
			if (newState.get("turn") == "S") turnId = yourColor == "W" ? yourPlayerId : opponentPlayerId;
			else turnId = newState.get("turn") == yourColor ? yourPlayerId : opponentPlayerId;
		}
		
		if (newState.size() == 1){ //initial state, contains only one record("turn": "W/B")
		...
		if (isMyTurn()){ 
			if (yourColor == "W") choosePosition(1,2);
			if (yourColor == "B") choosePosition(1,1);
		}
	}
	...
	public void positionSelected(String position){
		check(isMyTurn());
		int color;
		if (yourColor == "W") color=1; else color=2;
		if ("" == origin) {origin = position;choosePosition(2,0);} // origin selected, continue;
		else{
			if (position == origin) {origin="";choosePosition(1,color);} // origin canceled, continue;
			else {
